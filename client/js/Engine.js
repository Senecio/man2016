...
function FillCanvasToFullWindowHeight(canvas)
{
    var ctx = canvas.getContext("2d");
    canvas.height = typeof (init_window_innerHeight) === 'number' ? init_window_innerHeight : window.innerHeight;
...
engine.InitCanvas = function(canvas, fullCanvas)
{
    ...
    if(screenfull.enabled){
        document.addEventListener(screenfull.raw.fullscreenchange, function() { window.setTimeout(function() { FullScreenChangeCanvas(canvas); },  500); });
    }
    
    if(fullCanvas === true){
        FillCanvasToFullWindowHeight(canvas);
    }
}
